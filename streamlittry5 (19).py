...
if st.sidebar.button("Run Analysis"):
    ...
    if hist.empty:
        ...
        st.markdown(report)
        # ================= 当日期权成交量与价格分布 =================
        st.subheader("📊 Option Volume & Price (by Strike)")
        volume_expiry = st.selectbox("Select Expiry for Volume Analysis", expirations)

        if st.button("Analyze Option Volume & Price"):
            chain_vol = stock.option_chain(volume_expiry)
            calls_vol, puts_vol = chain_vol.calls, chain_vol.puts

            # 成交均价近似处理（使用lastPrice近似VWAP）
            calls_vol['vwap'] = calls_vol['lastPrice']
            puts_vol['vwap'] = puts_vol['lastPrice']

            # 换手率 (成交量 / OI)
            calls_vol['turnover'] = calls_vol['volume'] / (calls_vol['openInterest'] + 1e-6)
            puts_vol['turnover'] = puts_vol['volume'] / (puts_vol['openInterest'] + 1e-6)

            # 绘制图形
            fig, ax = plt.subplots(3, 1, figsize=(10, 12), sharex=True)

            # 1️⃣ 成交量分布
            ax[0].bar(calls_vol['strike'], calls_vol['volume'], color='green', alpha=0.6, label='Call Volume')
            ax[0].bar(puts_vol['strike'], puts_vol['volume'], color='red', alpha=0.6, label='Put Volume')
            ax[0].set_title(f"Option Volume by Strike - {volume_expiry}")
            ax[0].set_ylabel("Volume")
            ax[0].legend()

            # 2️⃣ 成交价格分布
            ax[1].plot(calls_vol['strike'], calls_vol['vwap'], marker='o', color='green', label='Call Price')
            ax[1].plot(puts_vol['strike'], puts_vol['vwap'], marker='o', color='red', label='Put Price')
            ax[1].set_title("Option Last Price (VWAP Approximation)")
            ax[1].set_ylabel("Price ($)")
            ax[1].legend()

            # 3️⃣ 换手率
            ax[2].bar(calls_vol['strike'], calls_vol['turnover'], color='green', alpha=0.6, label='Call Turnover (Vol/OI)')
            ax[2].bar(puts_vol['strike'], puts_vol['turnover'], color='red', alpha=0.6, label='Put Turnover (Vol/OI)')
            ax[2].set_title("Turnover Rate by Strike (Volume ÷ OI)")
            ax[2].set_xlabel("Strike Price")
            ax[2].set_ylabel("Turnover Ratio")
            ax[2].legend()
        ...
        # Max Pain Path
        fig.add_trace(go.Scatter(x=days, y=max_path, mode='lines', name='Max Pain Path', line=dict(color='blue')))
        # BS Path
        fig.add_trace(go.Scatter(x=days, y=bs_path, mode='lines', name='BS Path', line=dict(color='orange')))
        # Average Path
        fig.add_trace(go.Scatter(x=days, y=avg_path, mode='lines', name='Average Path', line=dict(color='green', dash='dash')))
        # Forecast Bounds (Lower & Upper)
        fig.add_trace(go.Scatter(x=days, y=[forecast_low]*len(days), mode='lines', name='Forecast Lower Bound', line=dict(color='red', dash='dot')))
        fig.add_trace(go.Scatter(x=days, y=[forecast_high]*len(days), mode='lines', name='Forecast Upper Bound', line=dict(color='red', dash='dot')))

        # Layout settings
        fig.update_layout(
            title="Interactive Price Anticipation Plot",
            xaxis_title="Days",
            yaxis_title="Price",
            hovermode="x unified",  # 鼠标悬停时显示所有曲线的值
            template="plotly_white"
        )

        # Show Plotly interactive chart in Streamlit
        ...
        # ========== Interactive Price Projection + Max OI Range ==========
        fig = go.Figure()

        # 价格预测路径
        fig.add_trace(go.Scatter(x=days, y=max_path, mode='lines', name='Max Pain Path', line=dict(color='blue', width=2)))
        fig.add_trace(go.Scatter(x=days, y=bs_path, mode='lines', name='BS Path', line=dict(color='green', width=2)))
        fig.add_trace(go.Scatter(x=days, y=avg_path, mode='lines', name='Average Path', line=dict(color='purple', dash='dash', width=2)))

        # 当前价与参考线
        fig.add_trace(go.Scatter(x=days, y=[current_price]*len(days), mode='lines', name='Current Price', line=dict(color='gray', dash='dot')))
        fig.add_trace(go.Scatter(x=days, y=[forecast_low]*len(days), mode='lines', name='Forecast Lower Bound', line=dict(color='red', dash='dot')))
        fig.add_trace(go.Scatter(x=days, y=[forecast_high]*len(days), mode='lines', name='Forecast Upper Bound', line=dict(color='red', dash='dot')))
        fig.add_trace(go.Scatter(x=days, y=[avg_max_pain]*len(days), mode='lines', name='Avg Max Pain', line=dict(color='blue', dash='dot')))
        fig.add_trace(go.Scatter(x=days, y=[bs_implied_price_call]*len(days), mode='lines', name='BS Implied Price (Call)', line=dict(color='magenta', dash='dashdot')))
        fig.add_trace(go.Scatter(x=days, y=[bs_implied_price_put]*len(days), mode='lines', name='BS Implied Price (Put)', line=dict(color='brown', dash='dashdot')))

        # Max OI 上下限
        fig.add_trace(go.Scatter(x=days, y=[max_call]*len(days), mode='lines', name=f"Max Call OI Strike (${max_call:.2f})", line=dict(color='green', dash='dot')))
        fig.add_trace(go.Scatter(x=days, y=[max_put]*len(days), mode='lines', name=f"Max Put OI Strike (${max_put:.2f})", line=dict(color='red', dash='dot')))

        # 区间填充
        fig.add_trace(go.Scatter(
            x=list(days)+list(days[::-1]),

            y = list([max_put]*len(days)) + list(([max_call]*len(days))[::-1]),

            fill='toself', fillcolor='gray', opacity=0.08, line=dict(width=0),
            name='Max OI Range'
        ))
        ...
        # 布局
        fig.update_layout(
            title="📈 Price Projection + Max OI Range",
        ...
        # ================== Comprehensive Option Strategy Generator ==================
        import numpy as np

        def generate_option_strategies(calls, puts, current_price, max_pain):
            strikes = sorted(calls['strike'].unique())
            atm = min(strikes, key=lambda x: abs(x - current_price))
            otm_call = min([s for s in strikes if s > atm], default=atm)
            otm_put = max([s for s in strikes if s < atm], default=atm)

            return [
                {"name": "Long Call", "legs": [(atm, "call", "long", calls.loc[calls['strike']==atm,'lastPrice'].iloc[0])]},
                {"name": "Short Call", "legs": [(otm_call, "call", "short", calls.loc[calls['strike']==otm_call,'lastPrice'].iloc[0])]},
                {"name": "Long Put", "legs": [(atm, "put", "long", puts.loc[puts['strike']==atm,'lastPrice'].iloc[0])]},
                {"name": "Short Put", "legs": [(otm_put, "put", "short", puts.loc[puts['strike']==otm_put,'lastPrice'].iloc[0])]},
                {"name": "Bull Call Spread", "legs": [
                    (atm, "call", "long", calls.loc[calls['strike']==atm,'lastPrice'].iloc[0]),
                    (otm_call, "call", "short", calls.loc[calls['strike']==otm_call,'lastPrice'].iloc[0])]},
                {"name": "Bear Put Spread", "legs": [
                    (atm, "put", "long", puts.loc[puts['strike']==atm,'lastPrice'].iloc[0]),
                    (otm_put, "put", "short", puts.loc[puts['strike']==otm_put,'lastPrice'].iloc[0])]},
                {"name": "Straddle", "legs": [
                    (atm, "call", "long", calls.loc[calls['strike']==atm,'lastPrice'].iloc[0]),
                    (atm, "put", "long", puts.loc[puts['strike']==atm,'lastPrice'].iloc[0])]},
                {"name": "Iron Condor", "legs": [
                    (otm_put, "put", "short", puts.loc[puts['strike']==otm_put,'lastPrice'].iloc[0]),
                    (otm_put-5, "put", "long", puts.loc[puts['strike']==otm_put-5,'lastPrice'].iloc[0]),
                    (otm_call, "call", "short", calls.loc[calls['strike']==otm_call,'lastPrice'].iloc[0]),
                    (otm_call+5, "call", "long", calls.loc[calls['strike']==otm_call+5,'lastPrice'].iloc[0])]},
                {"name": "Butterfly", "legs": [
                    (atm-5, "call", "long", calls.loc[calls['strike']==atm-5,'lastPrice'].iloc[0]),
                    (atm, "call", "short", calls.loc[calls['strike']==atm,'lastPrice'].iloc[0]),
                    (atm+5, "call", "long", calls.loc[calls['strike']==atm+5,'lastPrice'].iloc[0])]}
            ]
        ...
        def calculate_pnl(strategy, price_range):
            ...
            for strike, opt_type, pos, premium in strategy:
                intrinsic = np.maximum(price_range-strike,0) if opt_type=="call" else np.maximum(strike-price_range,0)
                pnl_leg = intrinsic-premium if pos=="long" else premium-intrinsic
                pnl += pnl_leg
            return pnl

        def plot_pnl(strategy_name, strategy, price_range):
            pnl = calculate_pnl(strategy, price_range)
            fig, ax = plt.subplots(figsize=(8,4))
            ax.plot(price_range, pnl, label=strategy_name)
            ax.axhline(0, color='black', linestyle='--')
            ax.set_title(f"{strategy_name} P/L Diagram")
            ax.set_xlabel("Underlying Price ($)")
            ax.set_ylabel("Profit / Loss ($)")
            ax.legend()
            st.pyplot(fig)

        # Strategy reason mapping
        reason_map = {
            "Long Call": "Bullish bias with lower IV: directional upside bet.",
            "Short Call": "Neutral/bearish bias with high IV: collect premium.",
            "Long Put": "Bearish bias or hedge downside risk.",
            "Short Put": "Neutral/bullish with high IV: income strategy.",
            "Bull Call Spread": "Moderate bullish bias with controlled risk.",
            "Bear Put Spread": "Moderate bearish bias with controlled risk.",
            "Straddle": "Expect volatility expansion or breakout.",
            "Iron Condor": "High pinning probability or range-bound market.",
            "Butterfly": "Market expected to pin near Max Pain."
        }

        # Loop through expiries for strategy output
        for expiry in selected_expiries:
            st.subheader(f"Option Strategies & P/L Analysis: {expiry}")
            chain_exp = stock.option_chain(expiry)
            calls_exp, puts_exp = chain_exp.calls, chain_exp.puts
            pain_price = calculate_max_pain(calls_exp, puts_exp)

            strategies = generate_option_strategies(calls_exp, puts_exp, current_price, pain_price)
            price_range = np.linspace(current_price*0.8, current_price*1.2, 200)

            for strat in strategies:
                st.write(f"### Strategy: {strat['name']}")
                st.write(f"**Reason:** {reason_map.get(strat['name'], 'General option play based on signals.')}")
                legs_text = ", ".join([f"{'Buy' if pos=='long' else 'Sell'} {strike}{'C' if opt_type=='call' else 'P'} (${premium:.2f})"
                                        for strike, opt_type, pos, premium in strat['legs']])
                st.write(f"Legs: {legs_text}")
                pnl = calculate_pnl(strat['legs'], price_range)
                max_profit = np.max(pnl)
                max_loss = np.min(pnl)
                st.write(f"**Max Profit:** ${max_profit:.2f} | **Max Loss:** ${max_loss:.2f}")
                plot_pnl(strat['name'], strat['legs'], price_range)